...
void create_graph_from_input(const char *file_name, vector<Node> &graph,
...
void write_output(const char *file_name, vector<Node> &graph, int k)
{
    std::ofstream ofs;
    ofs.open(file_name, std::ifstream::in);

    cout << endl
         << "前" << k << "个点的betweenness centrality：" << endl;
    printf("vetext\tBC");
    cout << endl;
    if (k < graph.size())
    {
        for (int i = 0; i < k; i++)
        {
            ...
            {
                printf("%d\t%.1f\n", graph.at(i).mNumber, graph.at(i).BC);
...
int main(int argc, char *argv[])
{
    ...
    write_output(argv[PARAM_OUT], graph, stoi(argv[PARAM_K]));
